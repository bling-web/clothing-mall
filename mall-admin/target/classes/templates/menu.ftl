        ...
        <div class="row">
            ...
            <div class="col-sm-5 widget-box widget-color-blue2">
                ...
                <div style="margin: 20px 20px;">
                    ${permission_attributes?seq_contains("/admin/menu/edit")?string("<button class='btn btn-info' onclick='addMenu()'>新增同级菜单</button> <button class='btn btn-info' onclick='addChildren()'>新增子级菜单</button>","")}
                    ${permission_attributes?seq_contains("/admin/menu/delete")?string(" <button class='btn btn-danger' onclick='del()'>删除菜单</button>","")}

                </div>
                <form class="form-horizontal" id="menuform" role="form" style="margin-top: 20px;">
                    <#--这两个是隐藏的表单项,在传数据给后台时使用,用户不需要看到-->
                    ...
                    ${(permission_attributes?seq_contains("/admin/menu/edit") || authorities?seq_contains("/admin/menu/delete"))?string("<button class='btn btn-info' type='button' style='margin-left: 20px;' onclick='add()'><i class='ace-icon fa fa-check bigger-110'></i>保存</button>","")}
...
<script type="text/javascript">
    jQuery(function($){
        ...
        $(".icon").click(function () {
         ...
         $('#tree1')
            // 注释的只是模板中的方法,在本项目并没有用处
    //      .on('loaded.fu.tree', function(e) {
	// })
    //      .on('updated.fu.tree', function(e, result) {
    //
	// })
         // 这个就是点击时,输入框中出现对应菜单名称的方法
         .on('selected.fu.tree', function(e,data) {
             console.log(data);
             //为当前表单设置parentid,以便传回后台时使用
	...
	// });
        ...
        //初始化的一个方法,也是增加子级菜单的关键
        function initiateDemoData(){

            //定义remoteDateSource方法
            var remoteDateSource = function(options, callback) {
                var self = this;
                // eval,js中的一个方法,就是接收字符串并执行,该方法只接收原始的字符串,不接受对象
                var json =  eval('${menu}');
                var $data = null;
            ...
            //这是一个模板,也就是获取id是tree1的这个对象,然后调用ace_tree方法,访问树方法.
            $('#tree1').ace_tree({
                //调用remoteDateSource方法
